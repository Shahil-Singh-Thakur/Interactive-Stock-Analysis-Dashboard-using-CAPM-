...
import datetime
import capm_functions # type: ignore
...
try:
    ...
    col1, col2 = st.columns([1,1])
    with col1:
        st.markdown('### Daraframe head')
        st.dataframe(stocks_df.head(), use_container_width = True)
    with col2:
        st.markdown('### Daraframe tail')
    ...
    with col1:
        ...
        st.plotly_chart(capm_functions.interactive_chart(stocks_df))
    with col2:
        print(capm_functions.normalize(stocks_df))
        st.markdown('### Price of all the stocks after normalization')
        st.plotly_chart(capm_functions.interactive_chart(capm_functions.normalize(stocks_df)))
    ...
    st.write('Plase select valid input')